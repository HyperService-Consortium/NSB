package system_merkle_proof_test
...
func Test_tenStorageHandler_GetStorageAt(t *testing.T) {
	type fields struct {
		handler system_merkle_proof.NSBClient
	}
	type args struct {
		typeID          uip.TypeID
	...
	c := nsbcli.NewNSBClient("http://121.89.200.234:26657")
	addr := sugar.HandlerError(hex.DecodeString("76071c08f94940ad53f35bf1474610ae99569ec10e257be1426f19347d24667d")).([]byte)
	tests := []struct {
		name    string
		fields  fields
		args    args
		...
		{name: "test_short_string", fields: fields{
			...
			contractAddress: addr,
			pos:             prover.NSBGetSlot(addr, []byte{}),
			description:     []byte("district"),
		}, want: uip.VariableImpl{Type: value_type.String, Value: "chain1"}},
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			eth := system_merkle_proof.TendermintNSBStorageHandler{
